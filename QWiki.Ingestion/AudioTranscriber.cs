...
public class AudioTranscriber
{
    ...
    private async Task EnsureFfmpegAsync()
    {
        ...
        // Xabe.FFmpeg.Downloader auto-downloads ffmpeg binaries to a local folder
        var ffmpegDir = Path.Combine(AppContext.BaseDirectory, "ffmpeg");
        Directory.CreateDirectory(ffmpegDir);

        _logger.LogInformation("Ensuring FFmpeg is available at {Path}...", ffmpegDir);
        await FFmpegDownloader.GetLatestVersion(FFmpegVersion.Official, ffmpegDir);
        FFmpeg.SetExecutablesPath(ffmpegDir);

        _ffmpegInitialized = true;
        _logger.LogInformation("FFmpeg ready");
    }
